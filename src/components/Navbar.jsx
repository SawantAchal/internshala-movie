          ...
          <div className="md:flex flex-col lg:flex-row items-center ">
            <div className="flex flex-col lg:flex-row">
              ...
              <select value={languageFilter} onChange={handleLanguageChange} className="p-2 rounded-md bg-transparent text-white focus:outline-none mb-2 lg:mb-0 lg:mr-2 border border-black">
                <option value="" className="bg-zinc-200 text-black">All Languages</option>
                {languages.map(language => (
                  <option key={language} value={language} className='bg-zinc-200 text-black'>
                    {language}
                  </option>
                ))}
              </select>
              ...
              <select value={countryFilter} onChange={handleCountryChange} className="p-2 rounded-md bg-transparent text-white focus:outline-none mb-2 lg:mb-0 lg:mr-2 border border-black">
                <option value="" className="bg-zinc-200 text-black">All Countries</option>
                {countries.map(country => (
                  <option key={country} value={country} className='bg-zinc-200 text-black'>
                    {country}
                  </option>
                ))}
              </select>
              ...
              <select value={genreFilter} onChange={handleGenreChange} className="p-2 rounded-md bg-transparent text-white focus:outline-none mb-2 lg:mb-0 lg:mr-2 border border-black">
                <option value="" className='bg-zinc-200 text-black'>All Genres</option>
                {genres.map(genre => (
                  <option key={genre} value={genre} className='bg-zinc-200 text-black'>
                    {genre}
                  </option>
                ))}
              </select>
              <button className='className="px-4 py-2  text-white rounded-md transition-colors border border-black  focus:outline-none lg:ml-2 p-1 bg-gradient-to-r from-red-700 to-gray-900 hover:bg-gradient-to-r hover:from-gray-900 hover:to-red-700' onClick={clearFilters}>Clear Filters</button>
            </div>
          </div>
        </div>
      </div>
